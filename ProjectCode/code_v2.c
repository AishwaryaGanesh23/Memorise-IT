...
void game_menu()//start game
{
    ...
    fseek(fd, 0, SEEK_END);
    fprintf(fd, "%s\t\t%d", uname,score);//enter score into file
    fclose(fd);
...
{
	int row, col, random, score=10,status=0,timer=1;//start score = 10
	...
	do
	{
		...
		//delay_time(20);//wait 2 seconds and then clear
		...
		delay_time(1);//wait 1 second
		system("clear");
		
		status=check_numbers(num);
	}while(status!=9);//change to time over
...
void delay_time(int seconds)//adds a delay 
{
